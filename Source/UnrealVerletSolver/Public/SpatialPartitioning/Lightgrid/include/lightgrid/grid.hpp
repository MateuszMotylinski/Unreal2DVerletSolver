...
namespace lightgrid {
    ...
    */    
    ...
    template<class T>
    void grid<T>::init(int iWidth, int iHeight, int iCell_size) {
    ...
    inline void grid<T>::cellRemove(int cell_node, int element_node) {
		...
		while (this->cell_nodes[current_node].element != element_node) {
			...
			if (current_node == -1 || previous_node == -1)
			{
				return;
			}
		}

		//for (auto entryNode : this->cell_nodes)
  //      for (int i = 0; i < this->cell_nodes.size(); ++i)
  //      {
  //          auto entryNode = this->cell_nodes[i].next;
  //          if (entryNode == element_node)
  //          {
		//		previous_node = current_node;
		//		current_node = this->cell_nodes[current_node].next;
  ...
  //              break;
  //          }
  //          //this->cell_nodes[current_node].element;
		//}
